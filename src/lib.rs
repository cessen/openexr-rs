...
/// Types used by OpenEXR to represent a value held by a particular channel at a particular point, suitable for being to
...
/// Types that represent the values of an arbitrary collection of channels at a particular point, suitable for being
/// written to directly by the decoder.
...
unsafe impl PixelStruct for (f32, f32, f32) {
    fn channels() -> &'static [(PixelType, usize)] {
        static TYPES: [(PixelType, usize); 3] =
            [(PixelType::FLOAT, 0), (PixelType::FLOAT, 4), (PixelType::FLOAT, 8)];
        &TYPES
    }
}
...
unsafe impl PixelStruct for (f32, f32, f32, f32) {
    fn channels() -> &'static [(PixelType, usize)] {
        static TYPES: [(PixelType, usize); 4] =
            [(PixelType::FLOAT, 0), (PixelType::FLOAT, 4), (PixelType::FLOAT, 8), (PixelType::FLOAT, 12)];
        &TYPES
    }
}
...
impl<'a> IStream<'a> {
    pub fn from_slice(slice: &[u8]) -> Self {
        IStream {
            handle: unsafe { CEXR_IStream_from_memory(b"in-memory data\0".as_ptr() as *const c_char,
...
impl<'a> InputFile<'a> {
    pub fn from_file(path: &Path) -> Result<InputFile<'static>> {
        let c_path = CString::new(path.to_str().expect("non-unicode path handlin is unimplemented").as_bytes()).unwrap();
        let mut error_out = ptr::null();
        let mut out = ptr::null_mut();
        let error = unsafe { CEXR_InputFile_from_file(c_path.as_ptr(), 1, &mut out, &mut error_out) };
        if error != 0 {
    ...
    pub fn from_stream(stream: &'a IStream) -> Result<InputFile<'a>> {
        let mut error_out = ptr::null();
        let mut out = ptr::null_mut();
        let error = unsafe { CEXR_InputFile_from_stream(stream.handle, 1, &mut out, &mut error_out) };
        if error != 0 {
            let msg = unsafe { CStr::from_ptr(error_out) };
            Err(Error::Generic(msg.to_string_lossy().into_owned()))
        } else {
    ...
    pub fn read_pixels(&self, framebuffer: &mut FrameBuffer) -> Result<()> {
        ...
        if (w.max.x - w.min.x) as usize != framebuffer.dimensions.0 - 1 || (w.max.y - w.min.y) as usize != framebuffer.dimensions.1 - 1 {
            panic!("framebuffer size {}x{} does not match input file dimensions {}x{}",
                   framebuffer.dimensions.0, framebuffer.dimensions.1,
                   w.max.x - w.min.x, w.max.y - w.min.y)
        }
        unsafe { CEXR_InputFile_set_framebuffer(self.handle, framebuffer.handle) };
        let mut error_out = ptr::null();
        let error = unsafe { CEXR_InputFile_read_pixels(self.handle, w.min.y, w.max.y, &mut error_out) };
        if error != 0 {
            let msg = unsafe { CStr::from_ptr(error_out) };
            Err(Error::Generic(msg.to_string_lossy().into_owned()))
        } else {
...
impl<'a> FrameBuffer<'a> {
    ...
    pub unsafe fn insert_raw(&mut self, name: &str, type_: PixelType,
                             base: *mut c_char,
                             stride: (usize, usize),
        ...
        CEXR_FrameBuffer_insert(self.handle, c_name.as_ptr(), type_, base,
                                stride.0, stride.1,
                                sampling.0, sampling.1,
                                fill_value,
                                tile_coords.0 as c_int, tile_coords.1 as c_int);
    }
    ...
    pub fn insert_channel<T: ChannelData>(&mut self, name: &str, fill: f64, data: &'a mut [T]) {
        if data.len() != self.dimensions.0 * self.dimensions.1 {
            panic!("data size of {} elements cannot back {}x{} framebuffer", data.len(), self.dimensions.0, self.dimensions.1);
        }
        let width = self.dimensions.0;
        unsafe { self.insert_raw(name, T::pixel_type(), data.as_mut_ptr() as *mut c_char,
    ...
    pub fn insert_pixels<'b, T: PixelStruct>(&mut self, channels: &[(&'b str, f64)], data: &'a mut [T]) {
        if data.len() != self.dimensions.0 * self.dimensions.1 {
            panic!("data size of {} elements cannot back {}x{} framebuffer", data.len(), self.dimensions.0, self.dimensions.1);
        }
        let width = self.dimensions.0;
        for (&(name, fill), &(ty, offset)) in channels.iter().zip(T::channels()) {
            unsafe { self.insert_raw(name, ty, (data.as_mut_ptr() as *mut c_char).offset(offset as isize),
                                     (mem::size_of::<T>(), width * mem::size_of::<T>()),
                                     (1, 1),
                                     fill,
                                     (false, false)) };
